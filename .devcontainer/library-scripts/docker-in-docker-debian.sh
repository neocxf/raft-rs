...
if [ "${USE_MOBY}" = "true" ]; then
    ...
    curl -fsSL https://mirrors.ustc.edu.cn/docker-ce/linux/${ID}/gpg | gpg --dearmor > /usr/share/keyrings/docker-archive-keyring.gpg
    echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://mirrors.ustc.edu.cn/docker-ce/linux/${ID} ${VERSION_CODENAME} stable" > /etc/apt/sources.list.d/docker.list
fi
...
if type docker > /dev/null 2>&1 && type dockerd > /dev/null 2>&1; then
    ...
    if [ "${USE_MOBY}" = "true" ]; then
        ...
        cat > /bin/docker-compose <<__EOF__
#!/bin/bash
